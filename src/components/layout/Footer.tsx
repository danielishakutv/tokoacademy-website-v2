'use client';

import Link from 'next/link';
import { navigation, contactInfo } from '@/data/config';

/**
 * The footer, on tokens.
 *
 * It used to be a fixed near-black slab with white text and a logo forced
 * white by `brightness-0 invert`. In a dark theme that slab sits against an
 * almost-identical page and the seam disappears; worse, the inverted logo
 * cannot be un-inverted for one theme without inverting it wrongly in the
 * other.
 *
 * So it is now a sunken surface: quiet grey under a light page, quiet
 * near-black under a dark one, separated by a rule either way. The logo goes
 * back to its own colours, which is the only version of it that is correct in
 * both themes — and the only version that is actually the brand.
 */
export default function Footer() {
  const currentYear = new Date().getFullYear();

  return (
    <footer className="border-t border-line bg-surface-sunken">
      <div className="section-container py-14 md:py-20">
        <div className="grid grid-cols-1 gap-10 sm:grid-cols-2 lg:grid-cols-5 lg:gap-12">
          {/* About */}
          <div className="sm:col-span-2">
            {/* eslint-disable-next-line @next/next/no-img-element */}
            <img
              src="/logo/toko-academy.png"
              alt="Toko Academy"
              width={445}
              height={144}
              className="mb-5 h-11 w-auto"
              loading="lazy"
              decoding="async"
            />
            <p className="prose-measure text-ink-muted">
              Empowering individuals and organizations with industry-relevant digital skills to thrive in the digital age.
            </p>
            <p className="mt-4 text-sm font-semibold text-ink">Skills for Tomorrow</p>
          </div>

          {/* Quick Links */}
          <div>
            <h3 className="eyebrow mb-4">Quick Links</h3>
            <ul className="space-y-1">
              {navigation.filter(item => !item.dropdown).map((item) => (
                <li key={item.name}>
                  {item.external ? (
                    <a
                      href={item.href}
                      target="_blank"
                      rel="noopener noreferrer"
                      className="link-hover flex min-h-[40px] items-center text-ink-muted"
                    >
                      {item.name}
                    </a>
                  ) : (
                    <Link href={item.href} className="link-hover flex min-h-[40px] items-center text-ink-muted">
                      {item.name}
                    </Link>
                  )}
                </li>
              ))}
            </ul>
          </div>

          {/* More */}
          <div>
            <h3 className="eyebrow mb-4">More</h3>
            <ul className="space-y-1">
              {navigation.find(item => item.dropdown)?.dropdown?.map((subItem: any) => (
                <li key={subItem.name}>
                  <Link href={subItem.href} className="link-hover flex min-h-[40px] items-center text-ink-muted">
                    {subItem.name}
                  </Link>
                </li>
              ))}
            </ul>
          </div>

          {/* Contact */}
          <div>
            <h3 className="eyebrow mb-4">Contact Us</h3>
            <ul className="space-y-3 text-ink-muted">
              <li className="flex items-start gap-2.5">
                <svg className="mt-1 h-5 w-5 shrink-0 text-brand" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" aria-hidden><path strokeLinecap="round" strokeLinejoin="round" d="M22 16.92v3a2 2 0 01-2.18 2 19.79 19.79 0 01-8.63-3.07 19.5 19.5 0 01-6-6A19.79 19.79 0 012.08 4.18 2 2 0 014 2h3a2 2 0 012 1.72c.12.9.33 1.78.63 2.62a2 2 0 01-.45 2.11L8.09 9.91a16 16 0 006 6l1.46-1.12a2 2 0 012.11-.45c.84.3 1.72.51 2.62.63A2 2 0 0122 16.92z" /></svg>
                <span>
                  <a href={`tel:${contactInfo.phones[0]}`} className="link-hover block">
                    {contactInfo.phones[0]}
                  </a>
                  <a href={`tel:${contactInfo.phones[1]}`} className="link-hover block">
                    {contactInfo.phones[1]}
                  </a>
                </span>
              </li>
              <li className="flex items-start gap-2.5">
                <svg className="mt-1 h-5 w-5 shrink-0 text-brand" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" aria-hidden><path strokeLinecap="round" strokeLinejoin="round" d="M4 4h16a2 2 0 012 2v12a2 2 0 01-2 2H4a2 2 0 01-2-2V6a2 2 0 012-2z" /><path strokeLinecap="round" strokeLinejoin="round" d="M22 6l-10 7L2 6" /></svg>
                {/* `break-all`: the address is one long token and would push the
                    column past the viewport on a 360px screen. */}
                <a href={`mailto:${contactInfo.email}`} className="link-hover break-all">
                  {contactInfo.email}
                </a>
              </li>
              <li className="flex items-start gap-2.5">
                <svg className="mt-1 h-5 w-5 shrink-0 text-brand" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" aria-hidden><path strokeLinecap="round" strokeLinejoin="round" d="M12 21s-8-6.5-8-12a8 8 0 1116 0c0 5.5-8 12-8 12z" /><circle cx="12" cy="9" r="2.5" /></svg>
                <span>{contactInfo.address}</span>
              </li>
            </ul>

            <div className="mt-6 flex flex-wrap items-center gap-4 text-ink-subtle">
              <a href={contactInfo.socialMedia.facebook} target="_blank" rel="noopener noreferrer" className="link-hover" aria-label="Facebook">
                <svg className="h-5 w-5" fill="currentColor" viewBox="0 0 24 24" aria-hidden><path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"/></svg>
              </a>
              <a href={contactInfo.socialMedia.instagram} target="_blank" rel="noopener noreferrer" className="link-hover" aria-label="Instagram">
                <svg className="h-5 w-5" fill="currentColor" viewBox="0 0 24 24" aria-hidden><path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zm0-2.163c-3.259 0-3.667.014-4.947.072-4.358.2-6.78 2.618-6.98 6.98-.059 1.281-.073 1.689-.073 4.948 0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98 1.281.058 1.689.072 4.948.072 3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98-1.281-.059-1.69-.073-4.949-.073zm0 5.838c-3.403 0-6.162 2.759-6.162 6.162s2.759 6.163 6.162 6.163 6.162-2.759 6.162-6.163c0-3.403-2.759-6.162-6.162-6.162zm0 10.162c-2.209 0-4-1.79-4-4 0-2.209 1.791-4 4-4s4 1.791 4 4c0 2.21-1.791 4-4 4zm6.406-11.845c-.796 0-1.441.645-1.441 1.44s.645 1.44 1.441 1.44c.795 0 1.439-.645 1.439-1.44s-.644-1.44-1.439-1.44z"/></svg>
              </a>
              <a href={contactInfo.socialMedia.twitter} target="_blank" rel="noopener noreferrer" className="link-hover" aria-label="X (Twitter)">
                <svg className="h-5 w-5" fill="currentColor" viewBox="0 0 24 24" aria-hidden><path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z"/></svg>
              </a>
              <a href={contactInfo.socialMedia.linkedin} target="_blank" rel="noopener noreferrer" className="link-hover" aria-label="LinkedIn">
                <svg className="h-5 w-5" fill="currentColor" viewBox="0 0 24 24" aria-hidden><path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433c-1.144 0-2.063-.926-2.063-2.065 0-1.138.92-2.063 2.063-2.063 1.14 0 2.064.925 2.064 2.063 0 1.139-.925 2.065-2.064 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z"/></svg>
              </a>
              <a href={contactInfo.socialMedia.whatsapp} target="_blank" rel="noopener noreferrer" className="link-hover" aria-label="WhatsApp">
                <svg className="h-5 w-5" fill="currentColor" viewBox="0 0 24 24" aria-hidden><path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413Z"/></svg>
              </a>
            </div>
          </div>
        </div>
      </div>

      {/* Bottom bar */}
      <div className="border-t border-line">
        <div className="section-container py-6">
          <div className="flex flex-col gap-4 text-sm text-ink-subtle md:flex-row md:items-center md:justify-between">
            <div className="space-y-1">
              {/*
                The registration line only. The address and both phone numbers
                used to be repeated here, directly beneath the Contact Us column
                that already lists them — the same details twice, a few
                centimetres apart, which reads as a mistake rather than as
                emphasis.
              */}
              <p>Toko Academy Ltd. — Registered with the Corporate Affairs Commission of Nigeria, 2025 (CAC).</p>
              {/*
                The year is read from the clock, so the copy baked in at build
                time and the copy rendered in the browser disagree from the first
                of January until the next deploy — a hydration mismatch that
                would appear on every page of the site and go unnoticed, because
                nobody rebuilds a static site on New Year's Day to check.
                suppressHydrationWarning tells React this one difference is
                expected rather than a bug.
              */}
              <p suppressHydrationWarning>© {currentYear} Toko Academy Ltd. All rights reserved.</p>
            </div>
            <nav className="flex flex-wrap gap-x-6 gap-y-2" aria-label="Legal">
              <Link href="/privacy" className="link-hover">Privacy Policy</Link>
              <Link href="/terms" className="link-hover">Terms of Service</Link>
              <Link href="/safeguarding" className="link-hover">Safeguarding Policy</Link>
            </nav>
          </div>
        </div>
      </div>
    </footer>
  );
}
